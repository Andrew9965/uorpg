    ...
    <table class="table table-condensed table-hover">
        ...
        @foreach($subcategories as $list)
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form id="delete{{$list->id}}"
                                  action="{{route('sub_category.destroy',[$list->id])}}"
                                  method="post">
                            </form>
                            <form id="update{{$list->id}}" method="POST"
                                  action="{{route('sub_category.update',[$list->id])}}"
                            ...
                            <div class="form-group{{ $errors->has('category_id') ? ' has-error' : '' }}">
                                ...
                                <fieldset>
                                    <select form="update{{$list->id}}" class="selectpicker"
                                            name="category_id" data-width="auto"
                                            data-live-search="true" data-size="10">
                                        @foreach($categories as $category)
                                            <option value="{{$category->id}}"
                                                    @if($list->category_id == $category->id) selected @endif>{{$category->title}}</option>
                                        @endforeach
                                    </select>
            ...
            <tr>
                ...
                <td>{{$categories->find($list->category_id)->title}}</td>
                {{--<td id="status">--}}
                    {{--@if($list->is_active == 1)--}}
                        {{--{{"Да"}}--}}
                    {{--@else--}}
                ...
                <td>@if($list->is_active == 0)
                        <form class="form-horizontal"
                              action="{{route('subcategory_active',[$list->id])}}"
                              method="POST">
