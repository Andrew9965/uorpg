<div class="table-responsive panel-body message">
    <table class="table table-condensed table-hover">
        ...
        <tr>
            ...
            <th class="col-md-2">@lang('user.category')</th>
            <th class="col-md-2">Фото</th>
        ...
        @foreach($subcategories as $list)
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form id="delete{{$list->id}}"
                                  action="{{route('sub_sub_category.destroy',[$list->id])}}"
                                  method="post">
                            </form>
                            <form id="update{{$list->id}}" method="POST"
                                  action="{{route('sub_sub_category.update',[$list->id])}}"
                            ...
                            <div class="form-group{{ $errors->has('subcategory_id') ? ' has-error' : '' }}">
                                ...
                                <fieldset>
                                    <select form="update{{$list->id}}" class="selectpicker"
                                            name="subcategory_id" data-width="auto"
                                            data-live-search="true" data-size="10">
                                        @foreach($categories as $category)
                                            <option value="{{$category->id}}"
                                                    @if($list->subcategory_id == $category->id) selected @endif>{{$category->title}}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="form-group{{ $errors->has('logo') ? ' has-error' : '' }}">
                                ...
                                <label class="btn btn-default btn-file no-results">
                                    {{trans('user.browse')}}<input form="update{{$list->id}}" type="file" class="imgInp"
                                                                   style="display: none;"
                                                                   name="logo"/>
                                </label>
                                @if ($errors->has('logo'))
                                ...
                                @endif
                                <img class="img-responsive img-thumbnail" style="display: none;" id="thumbnail-{{$list->id}}"
                                     src="" alt="your image"/>
                            </div>
            ...
            <tr>
                ...
                <td>{{$categories->find($list->subcategory_id)->title}}</td>
                <td><img class="img-responsive img-thumbnail" onerror="this.onerror=null;this.src='{{asset("img/no-image.png")}}';"
                ...
                <td>@if($list->is_active == 0)
                        <form class="form-horizontal"
                              action="{{route('subsubcategory_active',[$list->id])}}"
                              method="POST">
